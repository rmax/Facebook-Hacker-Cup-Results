...
int solve(int64 t[])
{
	...
	for (int is_win=0;;is_win=1-is_win)
	{
		int key=-1;
		for (int i=1;i<n-1;i++) if (key>=0 || a[i]>a[key]) key=i;
		if (key<0 || a[key]<=0) return is_win;
		a[key-1]+=a[key];
		a[key+1]+=a[key];
		a[key]=-a[key];
	}
...
int main(int argc,char **args)
{
	...
	freopen("alien_game.txt","r",stdin); freopen("A.out","w",stdout);
	int testcase;
	cin>>testcase;
	for (int caseId=1;caseId<=testcase;caseId++)
	{
		...
		sort(g,g+size);
		reverse(g,g+size);
		g[size++]=-1000000000LL*1000000000LL;
		int64 last=-F;
