...
int main(int argc,char **args)
{
	...
	freopen("party_time.txt","r",stdin); freopen("C.out","w",stdout);
	int testcase;
	scanf("%d",&testcase);
	for (int caseId=1;caseId<=testcase;caseId++)
	{
		...
		for (int set=1;set<two(m);set++) for (int i=0;i<n;i++) d[set][i]=oo;
		for (int set=1;set<two(m);set++)
		{
			if (countbit(set)==1)
			{
				int k=0;
			...
			{
				for (int subset=1;subset<set;subset++) if ((set&subset)==subset)
					for (int i=0;i<n;i++)
						checkmin(d[set][i],d[subset][i]+d[set-subset][i]-w[i]);
			}
			bool visited[maxn];
			...
			{
				int key=-1;
				for (int i=0;i<n;i++) if (!visited[i] && (key<0 || d[set][i]<d[set][key])) key=i;
				if (key<0) break;
				if (d[set][key]>=oo) break;
				visited[key]=true;
				for (int i=0;i<n;i++) if (g[key][i])
					checkmin(d[set][i],d[set][key]+w[i]);
			}
		}
		int R=oo;
		for (int i=0;i<n;i++) checkmin(R,d[two(m)-1][i]);
		printf("%d %d\n",R/(n+1),R%(n+1));
		fflush(stdout);
	}
	return 0;
