...
void update(int x0,int y0,int z0)
{
	if (x0<0 || x0>1000 || y0<0 || y0>1000 || z0<0 || z0>1000) return;
	int state=(x0<<20)+(y0<<10)+z0;
	if (mask[state>>5]&((uint(1))<<(state&31)))
		return;
	mask[state>>5]|=((uint(1))<<(state&31));
	int E=2000000000;
	for (int i=0;i<n;i++)
	{
		int dx=x[i]-x0;
	...
	if (E>R) R=E;
	G.insert(MP(-R,state));
	while (SIZE(G)>200) G.erase(--G.end());
}
int main(int argc,char **args)
{
	if (0)
	{
		freopen("B2.in","w",stdout);
		printf("50\n");
		for (int i=0;i<50;i++)
		{
			printf("%d\n",200);
			for (int j=0;j<200;j++)
				printf("%d %d %d\n",rand()%1001,rand()%1001,rand()%1001);
			printf("\n");
		}
		return 0;
	}
	...
	freopen("safest_place.txt","r",stdin); freopen("B2.out","w",stdout);
	int testcase;
	scanf("%d",&testcase);
	for (int caseId=1;caseId<=testcase;caseId++)
	{
		...
		R=0;
		G.clear();
		for (int x0=0;x0<=1000;x0+=M) for (int y0=0;y0<=1000;y0+=M) for (int z0=0;z0<=1000;z0+=M)
			update(x0,y0,z0);
		vector<ipair> S(G.begin(),G.end());
		for (int i=0;i<SIZE(S);i++)
		{
			int x0=S[i].second>>20;
			int y0=(S[i].second>>10)&1023;
			int z0=S[i].second&1023;
			for (int dx=-M;dx<=M;dx++) for (int dy=-M;dy<=M;dy++) for (int dz=-M;dz<=M;dz++)
				update(x0+dx,y0+dy,z0+dz);
		}
		printf("%d\n",R);
		fflush(stdout);
	}
